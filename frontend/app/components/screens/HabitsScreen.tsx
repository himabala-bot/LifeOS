    ...
    <div className="max-w-6xl mx-auto space-y-8 pb-16">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
        <div className="p-4 sm:p-5 rounded-2xl bg-white border border-[var(--line)] shadow-xs">
          ...
          <p className="text-2xl sm:text-3xl font-bold mt-1 text-[var(--ink)]">{habits.length}</p>
        </div>
        <div className="p-4 sm:p-5 rounded-2xl bg-white border border-[var(--line)] shadow-xs">
          <p className="text-[10px] uppercase tracking-wider font-semibold text-[var(--muted)]">7-Day Consistency</p>
          <p className="text-2xl sm:text-3xl font-bold mt-1 text-[var(--sage)]">{overallConsistency}%</p>
        </div>
        <div className="p-4 sm:p-5 rounded-2xl bg-white border border-[var(--line)] shadow-xs">
          <p className="text-[10px] uppercase tracking-wider font-semibold text-[var(--muted)]">Longest Streak</p>
          <p className="text-2xl sm:text-3xl font-bold mt-1 text-[var(--accent)]">{maxStreak} days</p>
        </div>
        <div className="p-4 sm:p-5 rounded-2xl bg-white border border-[var(--line)] shadow-xs">
          <p className="text-[10px] uppercase tracking-wider font-semibold text-[var(--muted)]">Total Check-ins</p>
          <p className="text-2xl sm:text-3xl font-bold mt-1 text-[var(--ink)]">{totalLogs}</p>
        </div>
      </div>
      ...
      <form onSubmit={handleCreate} className="p-3.5 sm:p-4 bg-white rounded-2xl border border-[var(--line)] shadow-xs flex flex-col md:flex-row gap-3 items-stretch md:items-center">
        ...
        <div className="flex flex-wrap sm:flex-nowrap items-center gap-2 w-full md:w-auto">
          ...
          <button
            type="submit"
            className="w-full sm:w-auto px-5 py-2.5 rounded-xl bg-[var(--sage)] hover:bg-[#668560] text-white text-xs font-semibold uppercase tracking-wider flex items-center justify-center gap-1.5 shadow-sm cursor-pointer"
          >
            <Plus size={16} />
            <span>Create</span>
          </button>
      ...
      <div className="bg-white rounded-3xl p-4 sm:p-6 border border-[var(--line)] shadow-xs">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-4 border-b border-[var(--line)] mb-4 gap-2">
          <div>
            <h3 className="font-semibold text-base text-[var(--ink)]">Habit Matrix & Activity Heatmap</h3>
          ...
          <div className="flex items-center gap-4 text-xs font-semibold text-[var(--muted)]">
            ...
            <span className="flex items-center gap-1.5">
              <span className="w-3 h-3 rounded-full border border-[var(--line)] inline-block" /> Missed
            </span>
          </div>
        </div>
        ...
        {filteredHabits.length === 0 ? (
          <div className="py-14 text-center text-[var(--muted)]">
            <p className="text-sm font-medium">No habits registered in this view.</p>
          ...
          <div className="space-y-3.5">
            {filteredHabits.map((habit) => {
              ...
              return (
                <div key={habit.id} className="p-3.5 sm:p-4 rounded-2xl bg-[#f8f7f4] border border-[var(--line)] hover:border-[var(--accent)]/30 transition-all">
                  <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3.5">

                    <div className="flex items-start justify-between gap-2 min-w-[200px]">
                      <div>
                        ...
                        <div className="flex items-center gap-2 sm:gap-3 mt-1 text-xs text-[var(--muted)] flex-wrap">
                      ...
                      <div className="flex items-center gap-1 shrink-0 lg:hidden">
                        <button
                          ...
                          className="p-1.5 rounded-lg text-[var(--muted)] hover:text-[var(--ink)] hover:bg-white transition-colors cursor-pointer"
                          title="Edit Habit"
                        >
                          <Edit3 size={15} />
                        </button>
                          ...
                          >
                            ...
                            <div
                              className={`w-7 h-7 rounded-full flex items-center justify-center text-xs transition-all ${
                                  ...
                                  ? 'bg-[var(--accent)] text-white shadow-xs font-bold scale-105'
                    ...
                    <div className="hidden lg:flex items-center gap-1 shrink-0 justify-end">
                      <button
                        onClick={() => setEditingHabit(habit)}
                        className="p-1.5 rounded-lg text-[var(--muted)] hover:text-[var(--ink)] hover:bg-white transition-colors cursor-pointer"
                        title="Edit Habit"
